...
test('castle: through a wall — he jumps two toward her, she lands on the square he crossed; it ends the turn', () => {
  ...
  assert.deepEqual(castleLanding(s), { kingFrom: fromSquare('h5'), king: fromSquare('f5'), rookie: fromSquare('g5') });
...
test('castle: refused when a landing square is blocked, on a file at T1, and out of rook form', () => {
  const blocked = board({ rookie: 'a5', enemies: [E('h5', 'king'), E('g5', 'pawn')], kit: [['castle', 1]] });
