...
### An empty board is unmeasurable for placement cards (The Warren, 2026-09-06/07)
The Warren's finale read 16-34% for its own pair and looked unbeatable. The human line was five moves; what was long was the BOT'S BRANCHING. In `scripts/run-playtest/bots/mcts.ts`, `perCand = max(1, floor(rolloutCount / candidates.length))` splits one budget (160 at T5) across every legal candidate, and `legalCandidates` emits one Boulder candidate AND one Snare candidate PER EMPTY SQUARE. On a near-empty finale those two cards alone were ~90 of ~110 candidates, so every candidate got a single rollout: the bot was sampling, not searching. `ROLLOUT_TOPK = 3` and a `fastScore` blind to a sealed hole meant the right square was considered about 3/N of the time.

**Rule:** when a kit contains a card that targets EMPTY SQUARES (boulder, snare, scarecrow, the summons' spawn squares), carve the finale out of solid stone — aim for 13-17 empty squares, not 45. The Warren's L7 went 34% -> 97% on that change alone, before any clock tuning, and the run then tuned cleanly into the band (69/59/69/63). The player's puzzle is unchanged; only the search space shrinks. A low pair number on an open board is evidence about the harness, not about the level.
